...
This project was initialized using the Remix Blues Stack. Learn more about [Remix Stacks](https://remix.run/stacks).
...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create rtemp-1b5e
  fly apps create rtemp-1b5e-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app rtemp-1b5e
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app rtemp-1b5e-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```console
  > WARN app flag 'rtemp-1b5e-staging' does not match app name in config file 'rtemp-1b5e'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name rtemp-1b5e-db
  fly postgres attach --app rtemp-1b5e rtemp-1b5e-db

  fly postgres create --name rtemp-1b5e-staging-db
  fly postgres attach --app rtemp-1b5e-staging rtemp-1b5e-staging-db
  ```
